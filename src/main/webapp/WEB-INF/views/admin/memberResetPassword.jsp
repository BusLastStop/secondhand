...
<style>
	...
	.list-container{height:80%;background-color:#f5f5f5; }
					    ...
					    <div class="video-contents">
						    <table style="width: 100%; border-collapse: collapse; text-align: center; background-color: #fff; box-shadow: 0px 4px 6px rgba(0, 0, 0, 0.1); border-radius: 8px; overflow: hidden;">
						        <thead style="background-color: #ff8a65; color: #fff;">
						        	<c:if test="${not empty resetPwList && resetPwList == null }">
							            <tr>
							                <th style="border: 1px solid #ddd; font-size: 11px;">번호</th>
							                <th style="border: 1px solid #ddd; font-size: 11px;">아이디</th>
							                <th style="border: 1px solid #ddd; font-size: 11px;">이름</th>
							                <th style="border: 1px solid #ddd; font-size: 11px;">생년월일</th>
							                <th style="border: 1px solid #ddd; font-size: 11px;">이메일</th>
							                <th style="border: 1px solid #ddd; font-size: 11px;">전화번호</th>
							                <th style="border: 1px solid #ddd; font-size: 11px;">신청날짜</th>
							                <th style="border: 1px solid #ddd; font-size: 11px;">해지신청</th>
							            </tr>
						            </c:if>
						        </thead>
						        <tbody>
						            <c:forEach var="resetPwList" items="${resetPwList}">
							            <tr style="background-color: #f9f9f9; transition: background-color 0.3s;">
							                <td style="border: 1px solid #ddd; font-size: 14px;"></td>
							                <td style="border: 1px solid #ddd; font-size: 14px;">${resetPwList.memId}</td>
							                <td style="border: 1px solid #ddd; font-size: 14px;">${resetPwList.memName}</td>
							                <td style="border: 1px solid #ddd; font-size: 14px;">${resetPwList.memBirthDate}</td>
							                <td style="border: 1px solid #ddd; font-size: 14px;">${resetPwList.memEmail}</td>
							                <td style="border: 1px solid #ddd; font-size: 14px;">${resetPwList.memPhone}</td>
							                <td style="border: 1px solid #ddd; font-size: 14px;">
							                	<fmt:formatDate value="${resetPwList.repCreated}" pattern="yyyy년 MM월 dd일"/>
							                </td>
							                <c:if test="${resetPwList.memResult == '신청'}">
							                	<td style="border: 1px solid #ddd; padding: 12px; font-size: 14px;">신청</td>
							                </c:if>
							                <c:if test="${resetPwList.memResult == '처리완료'}">
							                ...
							                <td style="border: 1px solid #ddd; padding: 12px; font-size: 14px;">
							                    <button onclick="location.assign('${path}/member/resetEnd/password.do?memId=${resetPwList.memId}&memEmail=${resetPwList.memEmail}')" style="border: 1px solid #ffc107; color: #ffffff; background-color: #6699ff; border-radius: 8px; padding: 6px 12px; cursor: pointer;">비밀번호 초기화</button>
							                </td>
							            </tr>
						            </c:forEach>
						        </tbody>
